...
class UserTest {
    ...
    @Test
    @DisplayName("Deve alterar a senha para um hash válido")
    void changePassword_deveAlterarParaHashValido() {
        User user = new User(VALID_NAME, VALID_EMAIL, VALID_PASSWORD_HASH, VALID_USERTYPE);
        String novoHash = "novoHashDeSenha123";
        user.changePassword(novoHash);
        assertEquals(novoHash, user.getPasswordHash());
    }

    @Test
    @DisplayName("Deve permitir alterar a senha para nulo")
    void changePassword_devePermitirSenhaNula() {
        User user = new User(VALID_NAME, VALID_EMAIL, VALID_PASSWORD_HASH, VALID_USERTYPE);
    ...
    @Test
    @DisplayName("Não deve permitir alterar senha para string vazia")
    void changePassword_naoDevePermitirSenhaVazia() {
        User user = new User(VALID_NAME, VALID_EMAIL, VALID_PASSWORD_HASH, VALID_USERTYPE);
