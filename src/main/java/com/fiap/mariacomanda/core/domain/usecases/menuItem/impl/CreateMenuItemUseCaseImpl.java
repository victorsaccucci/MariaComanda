...
public class CreateMenuItemUseCaseImpl implements CreateMenuItemUseCase {
    ...
    public MenuItem execute(CreateMenuItemInputDTO inputDTO, UUID requesterUserId) {
        ...
        if (inputDTO.description().trim().isEmpty()) {
        ...
        if (inputDTO.photoPath().trim().isEmpty()) {
            throw new ValidationException("Photo path cannot be empty");
        }
        if (!inputDTO.photoPath().matches("^/images/.*\\.(jpg|jpeg|png|gif|webp)$")) {
        ...
        if (inputDTO.price().compareTo(BigDecimal.ZERO) <= 0) {
            throw new ValidationException("Menu item price must be greater than zero");
        }
        ...
        Restaurant restaurant = restaurantGateway.findById(inputDTO.restaurantId())
                .orElseThrow(() -> new EntityNotFoundException("Restaurant not found", inputDTO.restaurantId().toString()));
